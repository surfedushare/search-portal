...
Cypress.Commands.add("searchFor", (search_term) => {
  cy.get("[data-test=search_term]").type(search_term).get("[data-test=search_button]").click();
});
...
Cypress.Commands.add("selectedFiltersShouldContain", (filter_name, filter_value, no_of_filters, filter_nr) => {
...
Cypress.Commands.add("selectFilter", (filter_category, filter_name) => {
  ...
  cy.get(`[data-category-id='${filter_category}'][data-item-id='${filter_name}']`).should("be.visible").click();
});
