    ...
    <div v-else-if="theme && !isLoading" class="theme">
      <div class="center_block center-header">
        <div class="theme__info ">
          <img src="/images/pictures/rawpixel-760027-unsplash.jpg"
      ...
      <div class="center_block theme__row">
        <div class="theme__description" >
          <h2>{{ $t('About-the-theme') }} <br>{{ getTitleTranslation(theme, $i18n.locale) }}</h2>
          <p><span class="html-content" v-html="getDescriptionTranslation(theme, $i18n.locale)"> </span></p>
        </div>
      ...
      <div class="theme__materials_and_communities theme__row center_block">
        ...
        <PopularList
          class="theme__communities"
          :communities="themeCommunities"
        >
          <template slot="header-info">
            <h2>{{ $t('Communities') }}</h2>
            <div class="popular-list__description">{{ $t('Subject-communities-within-the-theme') }} {{ getTitleTranslation(theme, $i18n.locale) }}</div>
          </template>
        </PopularList>
      </div>
      <div class="theme__collections theme__row center_block">
        <Materials
          ...
          <template slot="header-info">
            ...
            <p class="materials__description">{{ $t('Featured-learning-materials-in-the-theme') }} {{ getTitleTranslation(theme, $i18n.locale) }}</p>
          </template>
        </Materials>
      </div>
    </div>
...
<style scoped lang="less">
  ...
  .theme {
    ...
    &__description {
      ...
      &:before {
        content: '';
        background: url('/images/combined-shape.svg') no-repeat 0
        0;
        position: absolute;
        left: -23px;
        top: -40px;
