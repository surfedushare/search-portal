...
class TestCleanData(TestCase):
    ...
    def test_clean_data_duplicated_resources(self):
        ...
        for old_doc, new_doc in zip(oldest_version.document_set.all(), newest_version.document_set.all()):
            old_file_ids.append(old_doc.properties["pipeline"]["file"]["resource"][1])
            old_tika_ids.append(old_doc.properties["pipeline"]["tika"]["resource"][1])
            new_file_ids.append(new_doc.properties["pipeline"]["file"]["resource"][1])
            new_tika_ids.append(new_doc.properties["pipeline"]["tika"]["resource"][1])
